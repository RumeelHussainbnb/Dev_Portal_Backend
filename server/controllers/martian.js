...
export default {
  ...
  onGetMartians: async (req, res) => {
    try {
      ...
      const martianRegex = new RegExp(martian);
      ...
      result.data = await Martian.find({
        Expertise: { $regex: keywordRegex },
        Country: { $regex: countryRegex },
        MartianType: { $regex: martianRegex },
      })
        .skip(skip)
        .limit(perPage)
        .exec();
      totalMartains = await Martian.countDocuments({
        Expertise: { $regex: keywordRegex },
        Country: { $regex: countryRegex },
        MartianType: { $regex: martianRegex },
      }).exec();
